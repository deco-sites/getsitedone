...
const setup = ({ rootId, scroll, interval, infinite, isPerItem }: Props) => {
  ...
  const goToItem = (index: number, isInstant?: boolean) => {
    const item = items.item(index);
    ...
    slider.scrollTo({
      top: 0,
      behavior: isInstant ? "instant" : scroll,
...
function Slider({
  ...
  useEffect(() => setup({ rootId, scroll, interval, infinite, isPerItem }), [
    rootId,
    scroll,
    interval,
    infinite,
